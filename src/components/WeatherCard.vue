...
const windDirText = computed(() =>
  getWindDirection(props.current.wind_direction_10m)
);
  ...
  try {
    ...
    if (nearest) {
      ...
      if (
        ...
        currentStation.value.stationuuid !== nearest.stationuuid
      ) {
        currentStation.value = nearest;
        nextTick(() => {
          if (audioPlayer.value) {
            const url = nearest.url_resolved || nearest.url;
            audioPlayer.value.src = url.replace("http://", "https://");
            audioPlayer.value
              .play()
              .then(() => {
...
watch(
  () => [props.lat, props.lng, props.countryCode],
  () => {
...
watch(
  () => props.current.wind_direction_10m,
  () => {
    if (animationId) cancelAnimationFrame(animationId);
    initParticles();
  }
    ...
    <div class="details">
      ...
      <div class="detail-item wind-item">
        ...
        <span class="value"
          ...
          <span v-if="windDirText" class="wind-dir">{{
            windDirText
          }}</span></span
        >
      </div>
